...
void patient(int record_id)
{
    ...
    if(opt == 1){
        ...
        if(cho == 1){
            ...
            int ch;                              
            int state = 0; 
            while(line_count<MAX_LINE&&(ch=getchar())!=EOF){
                switch (state) {
                    case 0:
                    if (ch == '.') {
                        state = 1; 
                    ...
                    if (ch==','){
                        if(name_pos<MAX_NAME){
                            names[line_count][name_pos]='\0';
                        }else{
                            names[line_count][MAX_NAME-1]='\0';
                        }
                        line_count++;
                        name_pos=0;
                        state=2;
                    }else if(name_pos<MAX_NAME-1){
                        names[line_count][name_pos++]=ch;
                    }
                    break;
                    case2:
                    if (ch=='\n'){
...
//            char record_dor[MAX_NAME];
            if(target_line<line_count){
                strncpy(target_name,names[target_line],MAX_NAME);
            }
...
void chodoc(int arr[][20],int cho,int t,int cid,char dep[],char optdor[])
{
    ...
    printf("1.8:30-9:00剩余号数:%d\n2.9:00-9:30剩余号数:%d\n3.9:30-10:00剩余号数:%d\n4.10:00-10:30剩余号数:%d\n5.10:30-11:00剩余号数:%d\n6.11:00-11:30剩余号数:%d\n7.14:00-14:30剩余号数:%d\n8.14:30-15:00剩余号数:%d\n9.15:00-15:30剩余号数:%d\n10.15:30-16:00剩余号数:%d\n11.16:00-16:30剩余号数:%d\n",
        ...
        fprintf(fp,"%d%s%s%s",cid,dep,optdor,s);
        fclose(fp);
